...
function forceCleanupAllFaviconUrls() {
  ...
  if (
    ...
    Array.isArray(bookmarkManagerData.collections)
  ) {
    bookmarkManagerData.collections = bookmarkManagerData.collections.map(
      (collection) => {
...
window.addEventListener(
    ...
    if (e.target && e.target.tagName === "IMG") {
      ...
      if (
        ...
        (src.includes("faviconV2") ||
          src.includes("t2.gstatic.com") ||
          src.includes("t1.gstatic.com") ||
          src.includes("t3.gstatic.com"))
      ) {
  ...
  true,
); // Use capture phase to catch before it bubbles
...
function applyFilter(searchTerm) {
  ...
  collections.forEach((collectionElement) => {
    ...
    const collectionData = bookmarkManagerData.collections.find(
      (c) => c.id === collectionId,
    );
    ...
    if (isCollectionSearch) {
      ...
      bookmarkElements.forEach((b) =>
        b.classList.toggle("hidden", !showCollection),
      );
      ...
      bookmarkElements.forEach((bookmarkElement) => {
        ...
        const bookmarkData = collectionData.bookmarks.find(
          (b) => b.id === bookmarkId,
        );
        const bookmarkMatch = globalSearchTerms.some(
          (term) =>
      ...
      bookmarkElements.forEach((bookmarkElement) => {
        ...
        const bookmarkMatch = searchTerms.some(
          (term) =>
            bookmarkData.title.toLowerCase().includes(term) ||
            bookmarkData.url.toLowerCase().includes(term),
        );
    ...
    const bookmarksContainerElement =
      collectionElement.querySelector(".bookmarks");
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const backgroundThumbnailsContainer = document.getElementById(
  ...
  document
    .getElementById("toggleLeftPane")
    .addEventListener("click", function () {
      togglePane("leftPane");
    });

  document
    .getElementById("toggleRightPane")
    .addEventListener("click", function () {
  ...
  document.addEventListener("click", function (e) {
    if (window.innerWidth <= 768) {
      ...
      if (
        leftPane.classList.contains("open") &&
      ...
      if (
        rightPane.classList.contains("open") &&
  ...
  backgroundImages.forEach((imageName) => {
    ...
    thumbnailImg.addEventListener("click", () => {
      ...
      document
        .querySelectorAll(".custom-background-thumbnail")
        .forEach((thumb) => {
  ...
  if (!savedBackground && backgroundImages.length > 0) {
    ...
    const defaultThumbnail = backgroundThumbnailsContainer.querySelector(
      `.background-thumbnail[data-image-name="${defaultImageName}"]`,
    );
    if (defaultThumbnail) {
...
async function handleCustomBackgroundUpload(event) {
  ...
  try {
    ...
    if (Object.keys(customData.customImages).length >= 5) {
      alert(
        "Du kan ha max 5 anpassade bakgrundsbilder. Ta bort en befintlig först.",
      );
      return;
    }
...
function setCustomBackground(imageId) {
  ...
  const customThumbnail = document.querySelector(
...
function updateThumbnailSelection(imageId, type) {
  ...
  if (type === "custom") {
    ...
    const predefinedThumbnail = document.querySelector(
      `[data-image-name="${imageId}"]`,
    );
...
function mergeProperty(current, incoming) {
  return current === incoming ? current : current || incoming;
}
...
function generateUniqueCollectionName(baseName) {
  ...
  while (
    bookmarkManagerData.collections.some(
...
function createCollectionFromTabGroup(tabGroupData) {
  ...
  if (tabGroupData.tabs && Array.isArray(tabGroupData.tabs)) {
    ...
    tabGroupData.tabs.forEach((tab) => {
      ...
      if (tab.url === selfUrl || !tab.url || tab.url.startsWith("chrome://"))
        return;
...
function deleteAllCollections() {
  if (
    confirm(
      "Are you sure you want to reset ALL collections to default? This cannot be undone.",
    )
  ) {
  ...
  reader.onload = function (e) {
    try {
      ...
      const enrichedCollections =
        importedData.collections.map(enrichCollection);
    ...
    reader.onload = async function (e) {
      try {
        ...
        if (importedData.version === 3 && Array.isArray(importedData.lists)) {
          ...
          throw new Error(
            "Unsupported file format or version. Expected Toby v3 or v4.",
          );
        }
        ...
        const newCollections = await Promise.all(
          listsToImport.map(async (list) => {
            ...
            if (Array.isArray(list.cards)) {
              newCollection.bookmarks = await Promise.all(
                list.cards.map(async (card) => {
                  ...
                  return enrichBookmark({
                    title:
                      card.customTitle || card.title || "Untitled Bookmark", // Fallback för titel
                    url: card.url || "#", // Fallback för URL
                    description:
                      card.customDescription || card.description || "",
                    icon:
                      card.favIconUrl ||
                      (card.url
                        ? await getFavicon(card.url)
                        : "default-icon.png"), // Använd befintlig favIconUrl om den finns
                  });
                }),
              );
              newCollection.bookmarks = newCollection.bookmarks.filter(
                (b) => b !== null,
              ); // Ta bort null-värden (från överhoppade kort)
            }
            return newCollection;
          }),
        );
        ...
        if (validNewCollections.length > 0) {
          ...
          // Om det fanns listor men inga blev giltiga samlingar
          alert(
            "Bookmarks imported, but some items might have been invalid and were skipped.",
          );
        } else {
        ...
        console.error("Error importing bookmarks:", error);
        alert(
          "Error importing bookmarks. Please check the file format. Details: " +
            error.message,
        );
      }
    };
...
function saveToLocalStorage() {
  try {
    localStorage.setItem(
      "bookmarkManagerData",
      JSON.stringify(bookmarkManagerData),
    );
...
function checkAndShowFirstSyncNotification() {
  ...
  const hasShownFirstSync = localStorage.getItem(
    "hasShownFirstSyncNotification",
  );
...
function loadFromLocalStorage() {
  try {
    ...
    if (data) {
      ...
      bookmarkManagerData.rightPaneOpen =
        parsedData.rightPaneOpen !== undefined
          ? parsedData.rightPaneOpen
          : true;
    } else {
    ...
    document.getElementById("openInNewTab").checked =
      bookmarkManagerData.openInNewTab;
    document.getElementById("closeWhenSaveTab").checked =
      bookmarkManagerData.closeWhenSaveTab;
...
function renderCollections() {
  ...
  sortedCollections.forEach((collection) => {
    ...
    const buttons = [
      {
        className: "launch-collection",
        text: "🚀",
        title: "Open bookmarks in a Chrome group",
        action: () => launchCollection(collection.id),
      },
      {
        className: "openall-collection",
        icon: "outbox",
        title: "Open bookmarks in this collection",
        action: () => launchAllTabs(collection.id),
      },
      {
        className: "fetch-alltabs",
        icon: "inbox",
        title: "Get all Chrome tabs",
        action: () => fetchAllTabs(collection.id),
      },
      {
        className: "add-bookmark",
        text: "+",
        title: "Create bookmark",
        action: () => addBookmark(collection.id),
      },
      {
        className: "edit-collection",
        text: "✏️",
        title: "Edit collection",
        action: () => editCollection(collection.id),
      },
      {
        className: "edit-spaces",
        text: "🏷️",
        title: "Manage spaces for this collection",
        action: () => editCollectionSpaces(collection.id),
      },
      ...(bookmarkManagerData.collectionSortOrder === "userdefined"
        ? [
            {
              className: "move-collection",
              text: "▲",
...
function launchAllTabs(collectionId) {
  const collection = bookmarkManagerData.collections.find(
    (c) => c.id === collectionId,
  );
  if (collection) {
...
function createBookmarkElement(bookmark, collectionId) {
  ...
  bookmarkText.appendChild(bookmarkTitle);
  bookmarkText.appendChild(bookmarkDescription);
  ...
  bookmarkElement.addEventListener("click", () =>
    openBookmark(collectionId, bookmark.id),
  );
      ...
      if (faviconUrl) {
        ...
        findFavicon(baseUrl, function (faviconUrl) {
          if (faviconUrl) {
            ...
            console.log(
              `https://www.google.com/s2/favicons?domain=${cardUrl}&sz=32`,
            );
...
function enrichCollection(collection) {
  return {
    ...
    lastModified:
      collection.lastModified && Number(collection.lastModified)
...
function enrichSpace(space) {
  ...
  return {
    ...
    lastModified:
      space.lastModified && Number(space.lastModified)
        ? Number(space.lastModified)
        : Date.now(),
  };
}
...
function editCollectionSpaces(collectionId) {
  ...
  document.getElementById("saveSpaces").addEventListener("click", () => {
    const checkboxes = document.querySelectorAll(
      '#spacesDialog input[type="checkbox"]',
    );
...
function deleteCollection(collectionId) {
  if (confirm("Are you sure you want to delete this collection?")) {
    const collectionIndex = bookmarkManagerData.collections.findIndex(
      (c) => c.id === collectionId,
    );
    if (collectionIndex !== -1) {
      bookmarkManagerData.collections[collectionIndex].deleted = true;
      bookmarkManagerData.collections[collectionIndex].lastModified =
        Date.now();
      renderCollections();
    }
  }
  ...
  if (collection) {
    ...
    chrome.runtime.sendMessage(
      ...
      (response) => {
        if (chrome.runtime.lastError) {
          console.error(
            "Error launching collection:",
            chrome.runtime.lastError,
          );
          alert(
            "Error launching collection. Make sure the extension is installed and active.",
          );
...
function moveCollection(collectionId, direction) {
  const currentIndex = bookmarkManagerData.collections.findIndex(
    (c) => c.id === collectionId,
  );
  ...
  if (newIndex >= 0 && newIndex < bookmarkManagerData.collections.length) {
    const [movedCollection] = bookmarkManagerData.collections.splice(
      currentIndex,
      1,
    );
...
function dragStartCollection(e) {
  ...
  if (collectionElement) {
    ...
    e.dataTransfer.setData(
      "application/json",
      JSON.stringify({ type: "collection", id: collectionId }),
    );
...
function dragStartBookmark(e) {
  ...
  const collectionId =
    bookmarkElement.closest(".collection").dataset.collectionId;
  const bookmarkId = bookmarkElement.dataset.bookmarkId;
...
function dragOverCollection(e) {
  ...
  const collections = Array.from(
    collectionsContainer.querySelectorAll(".collection:not(.dragging)"),
  );
  ...
  if (!closestCollection) {
    ...
    closestCollection.parentNode.insertBefore(
      placeholder,
      closestCollection.nextSibling,
    );
  }
}
...
function dropCollection(e) {
  ...
  const droppedIndex = collections.findIndex(
    (c) => c.id === droppedCollectionId,
  );
  ...
  if (placeholder && placeholder.parentNode) {
    ...
    const adjustedIndex =
      placeholderIndex < droppedIndex ? placeholderIndex : placeholderIndex - 1;
...
function dragOverBookmark(e) {
  ...
  const allBookmarks = Array.from(container.children).filter((el) =>
...
function dropBookmark(e) {
  ...
  const fromCollection = bookmarkManagerData.collections.find(
    (c) => c.id === fromCollectionId,
  );
  const toCollection = bookmarkManagerData.collections.find(
    (c) => c.id === toCollectionId,
  );

  if (!fromCollection || !toCollection) return;

  const bookmarkIndex = fromCollection.bookmarks.findIndex(
    (b) => b.id === draggedItem.bookmarkId,
  );
  ...
  if (dropIndex === -1) {
    ...
    dropIndex = Math.floor(
      (yPos / containerRect.height) * toCollection.bookmarks.length,
    );
  }
...
function dropBookmarkContainer(e) {
  ...
  if (draggedItem) {
    ...
    if (draggedItem.type === "chromeTab") {
      ...
      if (fromCollection) {
        const movedBookmarkIndex = fromCollection.bookmarks.findIndex(
          (b) => b.id === fromBookmarkId,
        );
        if (movedBookmarkIndex !== -1) {
          const movedBookmark = fromCollection.bookmarks.splice(
            movedBookmarkIndex,
            1,
          )[0];
        ...
        response.forEach((windowData) => {
          ...
          for (const groupId in groupedTabs) {
            ...
            groupDragHandle.textContent =
              groupInfo && groupInfo.title ? groupInfo.title : "Tab Group"; // Exempelikon
            ...
            if (groupInfo && groupInfo.color) {
              ...
              const bgColor =
                colorMapping[groupInfo.color] || "rgba(0,0,0,0.1)";
    ...
    bookmarks: [
      {
        ...
        description:
          "🚀Thrust Me is a thrilling space adventure with danger and treasure!🌟🕹️",
      ...
      },
      {
        ...
        description:
          "A superfast, future-ready file manager powered by Everything.",
...
document
  .getElementById("importTobyFile")
  .addEventListener("change", importTobyBookmarks);
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.getElementById("zenMode").addEventListener("change", (e) => {
    ...
    if (window.innerWidth <= 768) {
      ...
      alert(
        "Zen mode is not available on mobile devices for optimal user experience.",
      );
      return;
    }
  ...
  document
    ...
    .addEventListener("click", deleteAllCollections);
  document
    .getElementById("cleanupFaviconsButton")
    .addEventListener("click", function () {
      forceCleanupAllFaviconUrls();
      saveToLocalStorage();
      renderCollections();
  ...
  document
    .getElementById("selectBackupFolder")
    .addEventListener("click", async () => {
      try {
        const currentFolder =
          bookmarkManagerData.autoBackup.customFolderName || "TheTabNinja";
        const customPath = prompt(
          "Enter subfolder name for backups (will be created in Downloads):",
          currentFolder,
        );
        ...
        if (customPath && customPath.trim()) {
          ...
          // Update UI
          document.getElementById("backupFolderPath").textContent =
  ...
  document
    .getElementById("collectionSortOrder")
  ...
  window.debugSpacesSync = function () {
    const collections = bookmarkManagerData.collections.filter(
      (c) => !c.deleted,
    );
    console.log("Collections with spaces:");
    collections.forEach((c) => {
      console.log(
        `Collection "${c.name}": spaces=${JSON.stringify(c.spaces)}, lastModified=${new Date(c.lastModified).toISOString()}`,
      );
    });
  };
  ...
  window.debugSorting = function () {
    ...
    collections.forEach((c) => {
      console.log(
        `"${c.name}": lastModified=${c.lastModified} (${new Date(c.lastModified).toISOString()}), position=${c.position}`,
      );
    });
    ...
    sorted.forEach((c, i) => {
      console.log(
        `${i + 1}. "${c.name}": lastModified=${c.lastModified} (${new Date(c.lastModified).toISOString()})`,
      );
    });
    ...
    collections.forEach((c) => {
      console.log(
        `"${c.name}": typeof lastModified = ${typeof c.lastModified}, isNaN = ${isNaN(c.lastModified)}, Number(lastModified) = ${Number(c.lastModified)}`,
      );
    });
  };
  ...
  async function syncToStorage() {
    try {
      await chrome.storage.local.set({
        bookmarkManagerData: bookmarkManagerData,
      });
    } catch (error) {
  ...
  function loadBackupSettings() {
    ...
    if (backupRetention) {
      backupRetention.value =
        bookmarkManagerData.autoBackup.keepDays.toString();
    }
    if (backupFolderPath) {
      const folderDisplay =
        bookmarkManagerData.autoBackup.useCustomFolder &&
        bookmarkManagerData.autoBackup.customFolderName
  ...
  document.getElementById("githubUsername").value =
    bookmarkManagerData.githubConfig.username || "";
    ...
    if (
      ...
      if (!closestCollection && !closestBookmarksContainer) {
        ...
        try {
          ...
          console.log(
            "Created new collection from tab group:",
            newCollection.name,
          );

          // Visuell feedback - blink den nya kollektionen
          setTimeout(() => {
            const newCollectionElement = document.querySelector(
              `[data-collection-id="${newCollection.id}"]`,
            );
            if (newCollectionElement) {
              newCollectionElement.style.transition =
...
function renderSpaces() {
  ...
  const activeSpaces = bookmarkManagerData.spaces.filter(
    (space) => !space.deleted,
  );
...
function addSpace() {
  ...
  const existingSpace = bookmarkManagerData.spaces.find(
    (s) => s.name === spaceName,
  );
  if (existingSpace) {
...
function deleteSpace(spaceName) {
  ...
  if (confirm(warningMsg)) {
    ...
    const spaceObj = bookmarkManagerData.spaces.find(
      (s) => s.name === spaceName,
    );
    if (spaceObj) {
...
function initializeSpaces() {
  ...
  const activeSpaceNames = bookmarkManagerData.spaces
    .filter((s) => !s.deleted)
    .map((s) => s.name);
  if (
    !bookmarkManagerData.currentSpace ||
...
function spaceDragOver(e) {
  ...
  if (draggedItem && draggedItem.type === "collection") {
    ...
    this.classList.add("drag-over");
    console.log(
      "✅ Drop allowed - dropEffect set to copy, target:",
  ...
  if (zenSearchBox) {
    zenSearchBox.addEventListener("input", function (event) {
      ...
      if (searchValue.trim().length > 0) {
        ...
        setTimeout(() => {
          mainSearchBox.focus();
          mainSearchBox.setSelectionRange(
            searchValue.length,
            searchValue.length,
          );
...
function handleZenKeyboard(event) {
  ...
  if (event.key === "Escape") {
    console.log(
      "ESC pressed in zen mode! (unique:",
      Date.now(),
      ") isSearching:",
    ...
    if (isSearching || hasScrolled) {
      console.log(
        "ENTERING SCROLL BRANCH! isSearching:",
        isSearching,
        "hasScrolled:",
        hasScrolled,
      );
